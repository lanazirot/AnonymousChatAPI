...
WORKDIR /src
COPY ["AnonymousChatAPI.csproj", "AnonymousChatAPI/"]
RUN dotnet restore "AnonymousChatAPI/AnonymousChatAPI.csproj"

WORKDIR "/src/AnonymousChatAPI"
COPY . .
RUN dotnet build "AnonymousChatAPI.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "AnonymousChatAPI.csproj" \
...
Execute unit tests
```
    cd "AnonymousChatAPI/test/AnonymousChatAPI.Tests"
    dotnet test
```

Deploy application
```
    cd "AnonymousChatAPI/src/AnonymousChatAPI"
